...
exports.createPersonnage = async (req, res) => {
    ...
    try {
        ...
        const personnage = await Personnage.create(req.body, {transaction});
        ...
        // on cherche si une arme de titre Poing éxiste
        let armeParDefaut = await Arme.findOne({
            where: {
                titre: "Poing",
            },
            transaction,
...
exports.updatePersonnage = async (req, res) => {
    ...
    try {
        const {id} = req.params;
        transaction = await sequelize.transaction();
        await Personnage.update(req.body, {
            where: {
                id,
            },
            transaction,
...
exports.updateArmes = async (req, res) => {
    let transaction;
    try {
        const {id} = req.params;
...
exports.updatePersonnageFromEvents = async (req, res) => {
    ...
    try {
        const {id} = req.params;
        const {events} = req.body;//une liste d'objets
        transaction = await sequelize.transaction();

        for (let i = 0; i < events.length; i++) {
            const event = events[i];
            const {which, id, type, operation, value} = event;
            // on regarde si c'est de type remove ou add
            // on regarde le which pour savoir si c'est un attribut, une arme ou un equipement
        }
