...
exports.createPersonnage = async (req, res) => {
    try {
        const result = await sequelize.transaction(async (t) => {
            const personnage = await Personnage.create(req.body, {transaction: t});
            ...
            await personnage.addArme(armeParDefaut, {transaction: t});
            return personnage;
        });
        if (result.error) {
            return res.status(result.code).json({error: result.error});
        }
        res.status(200).json({message: "Personnage updated"});
    } catch (error) {
        res.status(500).json({error: error.message});
    }
};

exports.getAllPersonnage = async (req, res) => {
    try {
...
exports.getOnePersonnage = async (req, res) => {
    try {
        const {id} = req.params;
        ...
        if (!personnage) {
            return res.status(404).json({error: "Personnage not found"});
        }
...
exports.updatePersonnage = async (req, res) => {
    try {
        const result = await sequelize.transaction(async (t) => {
            const {id} = req.params;
            const {force, dexterite, endurance, psychisme, resistance, initialized, nom} = req.body;
...
exports.updatePersonnageFromEvents = async (req, res) => {
    try {
        const result = await sequelize.transaction(async (t) => {
            const {id} = req.params;
            ...
            for (const event of events) {
                ...
                if (which === "attribute") {
                    ...
                    if (!equipement) {
                        return {
                            error: "Equipement not found", code: 404,
                        };
                    }
                    if (operation === "add") {
                    ...
                    return {
                        error: "Which not found", code: 400,
                    };
                }
    ...
    try {
        ...
        const result = await sequelize.transaction(async (t) => {
            ...
            if (!personnage) return {
                code: 404, error: "Personnage not found"
            }
