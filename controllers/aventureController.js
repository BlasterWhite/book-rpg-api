    ...
    try {
        ...
        const result = await sequelize.transaction(async (t) => {
            if (id_utilisateur == null) return {
                error: "id_utilisateur is required", code: 400,
            }

            if (id_livre == null) return {
                error: "id_livre is required", code: 400,
            }

            const livre = await Livre.findByPk(id_livre, {transaction: t});
            if (!livre) return {
                error: "id_livre not found", code: 404,
            }

            const section = await Section.findOne({
                where: {
                    id_livre, numero_section: 1,
                }, transaction: t
            });
            if (!section) return {
                error: "Section 1 for not found", code: 404,
            }
            ...
            if (!personnage) return {
                error: "id_personnage not found", code: 404,
            }
...
exports.getOneAventure = async (req, res) => {
    try {
        ...
        const aventure = await Aventure.findByPk(id, {
            include: [{
                model: Personnage, as: "personnage", separate: true, attributes: Personnage.getAttributes(),
            }, {
                model: Section, as: "section", separate: true, attributes: Section.getAttributes(),
            },],
        });
        res.status(200).json(aventure);
    } catch (error) {
...
exports.updateAventure = async (req, res) => {
    try {
        const {id} = req.params;
        const {id_section_actuelle, statut} = req.body;
        if (id_section_actuelle == null) return res.status(400).json({error: "id_section_actuelle is required"});
        if (statut == null) return res.status(400).json({error: "statut is required"});

        const aventure = await sequelize.transaction(async (t) => {
            const aventure = await Aventure.findByPk(id, {transaction: t});
            if (!aventure) return {error: "Aventure not found", code: 404};

            const section = await Section.findByPk(id_section_actuelle, {transaction: t});
    ...
    try {
        ...
        const aventure = await sequelize.transaction(async (t) => {
            ...
            await AssociationEquipementPersonnage.destroy({
                where: {id_personnage: aventure.id_personnage}, transaction: t
            });
