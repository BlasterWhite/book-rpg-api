...
exports.createLivre = async (req, res) => {
    try {
        const {titre, resume, id_image, tag, date_sortie, personnage} = req.body;

        const result = await sequelize.transaction(async (t) => {
            const [imageLivre, imageLivreCreated] = await Image.findOrCreate({
                where: {
                    id: id_image,
                },
                defaults: {
            ...
            const [personnageImages, personnageImagesCreated] = await Image.findOrCreate({
                where: {
            ...
            await personnageDefault.addArme(armeParDefaut, {transaction: t});
            return await Livre.create({
                    titre,
                    resume,
...
exports.updateLivre = async (req, res) => {
    try {
        const result = await sequelize.transaction(async (t) => {
            ...
            if (!livreExist) return {
                code: 404,
                error: "Book not found",
            }

            if (titre) livreExist.titre = titre;
            if (resume) livreExist.resume = resume;
            if (tag) livreExist.tag = tag;
            if (date_sortie) livreExist.date_sortie = date_sortie;
...
exports.deleteLivre = async (req, res) => {
    try {
        const {id} = req.params;
        const result = await sequelize.transaction(async (t) => {
            const livre = await Livre.findByPk(id, {transaction: t});
            if (!livre) return {
                code: 404,
                error: "Book not found",
            }
    ...
    try {
        const result = await sequelize.transaction(async (t) => {
            const livres = await sequelize.query(
                `SELECT l.id, l.titre, l.resume, l.id_image, l.tag, l.date_sortie
