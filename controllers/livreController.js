...
exports.createLivre = async (req, res) => {
    ...
    try {
        ...
        // on vérifie que les attributs de Personnage sont bien présent dans personnage
        if (personnage) {
            const {
                nom,
                description,
        ...
        const perso = await Personnage.create(personnage, {transaction});
        ...
        await perso.addArme(armeParDefaut, {transaction});
        const livre = await Livre.create({
            ...
            tag,
            date_sortie,
            id_personnage_default: perso.id
        }, {transaction});
